...
# 实际绘图
ggplot(resD, aes(y = Freq, x = res)) +  geom_histogram(binwidth = 1, stat="identity", fill = "steelblue") + xlab("该周大奖出现次数") + ylab ("周数") + ggtitle("直方图")
...
ggplot(resD, aes(x = res)) + geom_histogram(binwidth = 1, color = "white", fill = "steelblue") + xlab("该周大奖出现次数") + ylab ("周数") + ggtitle("直方图")
...
# 实际绘图
ggplot(dice, aes (x = 骰子)) + geom_histogram(binwidth = .1, fill = "steelblue", colour = "black", alpha = 0.5) + xlab("期望值") + ylab ("次数") + ggtitle("掷100次骰子后的期望值")
...
# 生成更精致的直方图
ggplot(dice, aes (x = 骰子)) + geom_histogram(binwidth = 1, fill = "steelblue", colour = "black", alpha = 0.5) + xlab("掷出的点数") + ylab ("次数") + ggtitle("掷10000次骰子的结果")
