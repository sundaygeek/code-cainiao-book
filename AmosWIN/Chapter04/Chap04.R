...
# 管道处理也可以用在生成图表上
fantuan %>%ggplot(aes(日期, 销售额)) + geom_line() + scale_x_date()  + ggtitle("饭团的销售额")
...
# 时间序列图
noodles %>% ggplot(aes(日期, 销售额)) + geom_line()+ facet_wrap(~品名)  + ggtitle("面条的销售额")
...
udon %>% ggplot(aes(乌冬面, 饭团)) + geom_point() + ggtitle("乌冬面与饭团的散点图")
...
# 时间序列图
milk2 %>% ggplot(aes(日期, 销售额)) + geom_line() + scale_x_date() + ggtitle("牛奶的销售额")
...
xy %>% ggplot(aes (x = X, y = Y)) + geom_point(size = 2) + ggtitle("看起来应该具有相关性")
...
# 画出气温与销售量的散点图
icecream %>% ggplot(aes(气温, 销售量)) + geom_point(size = 2)
...
icecream %>% ggplot(aes(气温, 销售量)) + geom_point(size = 2) + geom_smooth(method = "lm", se = FALSE)
...
heights %>% ggplot(aes(身高, 年收入)) + geom_point() + ggtitle("身高与年收入的相关性")
