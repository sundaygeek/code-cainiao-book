...
# 通过以下命令实际绘制图表
ggplot (breads, aes(x = weight)) + geom_histogram(binwidth = 10, fill = "steelblue", colour="black",  alpha = 0.5) + xlab("面包的重量") + ylab ("个数") + ggtitle("面包数据的直方图")  + xlim(360, 420)
...
ggplot(dice, aes (x = 骰子)) + geom_histogram(aes(y = ..density..),binwidth = .1, fill = "steelblue", colour = "black", alpha = 0.5) + xlab("期望值") + ylab ("") + ggtitle("骰子平均值的平均值") + stat_function(geom="line", fun = dnorm, args=list(mean = mean (dice$骰子), sd = sd (dice$骰子)))
